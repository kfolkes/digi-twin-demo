...
@description('Deploy IoT Hub for production telemetry ingestion. Set to true to provision IoT resources.')
...
// ===================== //
// IoT Hub (conditional, for production telemetry)
// ===================== //

module iotHub 'iot.bicep' = if (deployIoTHub) {
  name: 'iot-hub'
  params: {
    iotHubName: 'iot-${environmentName}-${resourceSuffix}'
    location: location
    tags: tags
    skuName: 'S1'
    principalId: managedIdentity.outputs.principalId
...
module containerAppApi 'br/public:avm/ptn/azd/acr-container-app:0.4.0' = {
  ...
  params: {
    ...
    env: union(
      ...
      deployIoTHub ? [
        { name: 'IOT_HUB_HOSTNAME', value: iotHub.outputs.hostName }
        { name: 'IOT_HUB_EVENT_HUB_ENDPOINT', value: iotHub.outputs.eventHubEndpoint }
        { name: 'AZURE_CLIENT_ID', value: managedIdentity.outputs.clientId }
      ] : []
    )
    secrets: [
...
output iotHubHostName string = deployIoTHub ? iotHub.outputs.hostName : ''
